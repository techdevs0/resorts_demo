  ...
  return (
    <div className='container'>
      ...
      <div className='room-suite-grid-wrapper'>
        {
          props.data?.map(x => (
            <RoomGridItem key={x.id} id={x.id} title={x.post_name} image={x.thumbnail} link={x.route} linkText={x.linkText} description={x.short_description} url={x.route} synx={x.post_url}/>
          ))
        }
      </div>
    </div>
